...
//  poi
//
//  Created by Teknasyon-S on 19.10.2018.
...
struct CategoryRequest {
    ...
    func makeRequest(closure: @escaping (MainModel) -> Void) {
        let req = CategoryRequest()
        var url = req.url
        url = url + "venues/categories?"
        ...
        Alamofire.request(url).responseJSON { response in
            ...
            if let data = response.data, let jsonString = String(data: data, encoding: .utf8) {
                ...
                do {
                    let model = try decoder.decode(MainModel.self, from: jsonData)
                    closure(model)
                } catch {
                    print(error)
                }
    ...
    func createQString(req: CategoryRequest) -> String {
        var qString = ""
